...
class ModelDetailsTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        if let keys = self.model?.attributes.keys {
             self.attributeNames = Array(keys)
             self.attributeNames?.sort()
        } else {
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if let names = self.attributeNames {
            return names.count
        }
        return 0
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "ModelAttributeCell", for: indexPath)
    ...
    private func setAttributeCell(cell:ModelAttributeCell, indexPath:IndexPath){
        ...
        if let name = self.attributeNames?[indexPath.row],
